...
export class SupabaseService {
  ...
  async updateVideoData({ videoRecordId, audioURL, images, captionJson }) {
    try {
      console.log("UpdateVideoData called with args:", videoRecordId);
  ...
  async getVideoById(videoId) {
    try {
      ...
      if (allVideosError) {
        ...
        throw new Error(`Cannot query video_data table: ${allVideosError.message}`);
      }
      
      console.log("All videos test successful, found:", allVideos?.length || 0, "videos");
  ...
  async deleteVideoData({ videoId, userId }) {
    try {
      ...
      console.error("DeleteVideoData error:", error);
      throw error;
    }
  }
