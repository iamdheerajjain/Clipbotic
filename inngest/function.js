      ...
      const updateResult = await step.run(
        ...
        async () => {
          try {
            ...
            console.log("Audio data:", GenerateAudioFile);
            console.log(
              "Audio URL extracted:",
              extractAudioURL(GenerateAudioFile)
            );
            console.log("GenerateImages count:", GenerateImages?.length || 0);
            console.log("GenerateImages data:", GenerateImages);
            console.log(
              "GenerateCaptions count:",
              GenerateCaptions?.length || 0
            );
            console.log("GenerateCaptions data:", GenerateCaptions);
            ...
            const result = await supabaseService.updateVideo(
              videoRecordId,
              updateData
            );

            if (!result) {
              throw new Error("updateVideo returned null");
            }
